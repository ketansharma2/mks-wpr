    ...
    <>
      ...
      >
        <div className="p-4 md:p-6 flex flex-col h-full overflow-hidden">
          {/* Logo & Brand Header */}
          <div
            className={`flex flex-col mb-6 pb-5 border-b border-slate-200 relative transition-all duration-300 ${!isHovered ? "md:items-center" : "items-start"} gap-3`}
          >
            ...
            {/* Company Name */}
            <div
              className={`transition-opacity duration-300 overflow-hidden text-center w-full ${!isHovered && "md:hidden"}`}
            >
          ...
          <nav className="space-y-2.5 text-sm font-medium flex-1">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-5 h-5 flex-shrink-0 transition-transform duration-300 ${isActive ? "scale-110 text-white" : "group-hover:scale-110 group-hover:text-blue-600 text-slate-500"}`}
                  />
                  <span
                    className={`tracking-wide text-sm sm:text-sm transition-opacity duration-300 ${!isHovered && "md:hidden"}`}
                  >
                    {item.label}
                  </span>
                </Link>
        ...
        <div
          ...
          title={
            !isHovered ? `${profileData.fullName} (MKS Member)` : undefined
          }
        >
          ...
          <div
            className={`transition-opacity duration-300 whitespace-nowrap overflow-hidden ${!isHovered && "md:hidden"}`}
          >
        ...
        <div className="p-4 md:p-6 border-t border-slate-200 bg-slate-50/60 overflow-hidden whitespace-nowrap">
          <button
            ...
            className={`flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-rose-600 hover:bg-rose-50 hover:text-rose-700 text-xs sm:text-sm font-semibold transition-all duration-300 ${
              !isHovered && "md:justify-center"
            }`}
          >
            <LogOut className="w-5 h-5 flex-shrink-0" />
            <span className={`${!isHovered && "md:hidden"}`}>Log Out</span>
          </button>
        </div>
      </aside>
            ...
            <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="border-t border-slate-200 pt-4 space-y-3">
                <div className="flex items-center gap-1.5 text-amber-600 text-xs font-bold uppercase tracking-wider">
                  <HelpCircle className="w-4 h-4" /> Help & Support (Report
                  Issue to Developer)
                </div>
                <p className="text-[11px] text-slate-500">
                  Facing any bug, error, or portal issue? Drop your message
                  below. It will be logged instantly for developer review.
                </p>
                  ...
                  <div className="flex items-center justify-between">
                    {issueSubmitted ? (
                      <span className="text-xs text-emerald-600 font-semibold animate-pulse">
                        ✓ Issue reported to developer successfully!
                      </span>
                    ) : (
                      <span className="text-[10px] text-slate-400">
                        Developer portal monitoring active.
                      </span>
                    )}
                    <button
                      type="submit"
