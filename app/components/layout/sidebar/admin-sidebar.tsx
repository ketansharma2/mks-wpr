...
export default function AdminSidebar() {
  ...
  const loadProfile = async () => {
    try {
      const res = await api.get("/admin/members/profile");
  ...
  const updateProfile = async () => {
    try {
      await api.put("/admin/members/profile", {
      ...
      >
        ...
        <div className="p-4 md:p-6 border-t border-slate-200 bg-slate-50/60 overflow-hidden whitespace-nowrap">
          <button
            ...
            className={`w-full flex items-center gap-3.5 px-4 py-3.5 rounded-2xl text-rose-600 hover:bg-rose-50 hover:text-rose-700 text-xs sm:text-sm font-semibold transition-all duration-300 ${
              !isHovered && "md:justify-center"
            }`}
          >
